...
class CeskapressPipeline:

    # Database setup
    conn = sqlite3.connect('ceskapress.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `ceskapress`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM ceskapress WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO ceskapress (date, title, link, content)"
